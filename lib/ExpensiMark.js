...

            {
                ...
                process: (textToProcess, replacement, shouldKeepRawInput = false) => {
                    ...
                    if (shouldKeepRawInput) {
                        ...
                        let replacedText = textToProcess.replace(rawInputRegex, replaceFunction);
                        
                        const lines = replacedText.split('\n');
                        
                        const originalLines = textToProcess.split('\n');
                        
                        const emptyBlockQuoteRegex = /<blockquote> *<\/blockquote>/;
                        const nonEmptyBlockQuoteRegex = /<blockquote>\s*\S.*?\s*<\/blockquote>/;
                        
                        replacedText = lines.map((line, index) => {
                            if (emptyBlockQuoteRegex.test(line)) {
                                let hasBeginQuote = false;
                                
                                for (let i = index - 1; i >= 0; i--) {
                                    if (nonEmptyBlockQuoteRegex.test(lines[i])) {
                                        hasBeginQuote = true;
                                        break;
                                    }
                        
                                    if (!emptyBlockQuoteRegex.test(lines[i])) {
                                        return originalLines[index];
                                    }
                                }
                        
                                for (let i = index + 1; i < lines.length; i++) {
                                    if (!nonEmptyBlockQuoteRegex.test(lines[i]) && !emptyBlockQuoteRegex.test(lines[i])) {
                                        return originalLines[index];
                                    }
                                }
                        
                                return hasBeginQuote ? line : originalLines[index];
                            }
